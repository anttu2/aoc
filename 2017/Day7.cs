...
namespace AdventOfCode
{
    public class Day7
    {
        ...
        [Theory]
        [InlineData(TestInput, "tknk")]
        public void TestInput_Part1(string notes, string nameOfRoot)
        {
            var rootProgram = GetRootProgram(notes);

            Assert.Equal(nameOfRoot, rootProgram.Name);
        }

        [Fact]
        public void RealInput_Part1()
        {
            var rootProgram = GetRootProgram(RealInput);

            _output.WriteLine(rootProgram.Name);
        }
        ...
        [Theory]
        [InlineData(TestInput, 60)]
        public void TestInput_Part2(string notes, int targetWeight)
        {
            var newWeight = GetCorrectWeightOfFaultyProgram(notes);

            Assert.Equal(targetWeight, newWeight);
        }

        [Fact]
        public void RealInput_Part2()
        {
            var newWeight = GetCorrectWeightOfFaultyProgram(RealInput);

            _output.WriteLine(newWeight.ToString());
        }

        #endregion

        private int GetCorrectWeightOfFaultyProgram(string notes)
        {
            var programs = ParseProgramNotes(notes);

            var unbalanced = programs.Values.Where(p => !p.IsSubTreeInBalance).OrderByDescending(p => p.Depth).First();
            var targetWeight = unbalanced.ChildPrograms.Except(new []{unbalanced}).First().TotalWeight;

            return targetWeight - unbalanced.UnequalChild.SubTreeWeight;
        }

        private Program GetRootProgram(string notes)
        {
            var programStructure = ParseProgramNotes(notes);
            return programStructure.Values.Single(p => p.Parent == null);
        }

       private IDictionary<string, Program> ParseProgramNotes(string notes)
       {
           var regex = new Regex("(?<name>\\w+) \\((?<weight>\\d+)\\)\\s?-?>?\\s?(?<childs>.*)$");
           ...
           var programDict = regexes.ToDictionary(r => r.Item2.Name, r => r.Item2);
           foreach (var match in regexes)
           {
               var childNames = match.Item1.Groups["childs"].Value
                   .Split(",")
                   .Select(c => c.Trim());

                programDict.Where(p => childNames.Contains(p.Key))
                    .ToList()
                    .ForEach(c =>
                    {
                        c.Value.Parent = match.Item2;
                        match.Item2.ChildPrograms.Add(c.Value);
                    });
           }
        ...
        public class Program
        {
            ...
            public IList<Program> ChildPrograms { get; } = new List<Program>();
            public Program Parent { get; set; }
            public int SubTreeWeight => ChildPrograms.Select(p => p.TotalWeight).Sum();
            public int TotalWeight => Weight + SubTreeWeight;
            public Program UnequalChild => ChildPrograms.Any() ? ChildPrograms.GroupBy(c => c.TotalWeight).First(g => g.Count() == 1)?.First() : null;
            public bool IsSubTreeInBalance => !ChildPrograms.Any() || ChildPrograms.Select(c => c.TotalWeight).Distinct().Count() == 1;
            ...
            public int Depth
            {
                ...
                {
                    int depth = 0;
                    Program.TraverseUp(this, p => depth++);

                    return depth;
                }
            }

            public static void TraverseUp(Program program, Action<Program> visitor)
            {
                visitor(program);
                if (program.Parent != null)
                    TraverseUp(program.Parent, visitor);
            }
        }
...
";
        ...
        private const string RealInput = @"yvpwz (50)
