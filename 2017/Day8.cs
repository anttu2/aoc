...
namespace AdventOfCode
{
    public class Day8
    {
        ...
        private static readonly Regex InstructionRegex = new Regex(@"(?<reg>\S+) (?<op>dec|inc) (?<val>-?\d+) if (?<comp_reg>\S+) (?<comp_op>[!=<>]+) (?<comp_val>-?\d+)");
...
c inc -20 if c == 10";
        ...
        public void TestInput_Part1(string instructions, int maxValueInAnyRegister)
        {
            var value = GetValueInLargestRegister(instructions);
            Assert.Equal(maxValueInAnyRegister, value.maxValue);
        }

        [Fact]
        public void RealInput_Part1And2()
        {
            var value = GetValueInLargestRegister(RealInput);

            _output.WriteLine($"MaxValue: {value.maxValue.ToString()} PeakValue: {value.peakValue}" );
        }

        private (int maxValue, int peakValue) GetValueInLargestRegister(string instructionsString)
        {
            var instructions = ParseInstructions(instructionsString).ToList();

            var registerDict = instructions
                .Select(i => i.Register)
                .Distinct()
            ...
            {
                instruction.Execute(registerDict);
                var peak = registerDict.Max(i => i.Value);
                peakValue = peak > peakValue 
                    ? peak 
                    : peakValue;
            }

            return (registerDict.Max(i => i.Value), peakValue);

        }

        private IEnumerable<Instruction> ParseInstructions(string instructions)
        {
            return instructions
                .Trim()
                .Split("\r\n")
                .Select(i =>
                {
                    ...
                    {
                        Register = match.Groups["reg"].Value,
                        Op = match.Groups["op"].Value == "inc" ? "+" : "-",
                        Value = int.Parse(match.Groups["val"].Value),
                        CompRegister = match.Groups["comp_reg"].Value,
                        CompOp = match.Groups["comp_op"].Value.Replace("==","=").Replace("!=", "<>"),
                        CompValue = match.Groups["comp_val"].Value
                    };
                });
        }
        ...
        class Instruction
        {
            ...
            public string CompRegister { get; set; }
            public string CompOp { get; set; }
            public string CompValue { get; set; }

            public string Expression => $"{Register} {Op} {Value}";
            public string CompExpression => $"{CompRegister} {CompOp} {CompValue}";

            public void Execute(Dictionary<string, int> registerDict)
            {
                ExpressionContext boolContext = new ExpressionContext();
                boolContext.Variables.Add(CompRegister, registerDict[CompRegister]);

                IGenericExpression<bool> e = boolContext.CompileGeneric<bool>(CompExpression);
                bool shouldExecute = e.Evaluate();

                if (shouldExecute)
                {
                    ExpressionContext opContext = new ExpressionContext();
                    opContext.Variables.Add(Register, registerDict[Register]);

                    IGenericExpression<int> ex = opContext.CompileGeneric<int>(Expression);
                    int newValue = ex.Evaluate();

                    registerDict[Register] = newValue;
                }
            }
        }
