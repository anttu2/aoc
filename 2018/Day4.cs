...
namespace BackmanConsulting.Aoc2018
{
    public class Day4
    {
        private class Guard
        {
            ...
            public IList<GuardEvent> Events { get; } = new List<GuardEvent>();
            public TimeSpan TotalSleep => Events.Where(e => e.Type == GuardEvent.TypeEnum.Sleeps).Aggregate(TimeSpan.Zero, (seed, e) => seed + e.Duration.Value);

            public (int index, int count) GetSleepiestMinute()
            {
                var sleepMinutes = new List<int>(new int[60]);
                foreach (var guardEvent in Events.Where(e => e.Type == GuardEvent.TypeEnum.Sleeps))
                {
                    var startMinute = guardEvent.StartTime.Minute;
                    var endMinute = guardEvent.EndTime.Value.Minute;
                ...
                var max = sleepMinutes.Max();
                return (sleepMinutes.FindIndex(i => i == max), max);
            }
        }
        ...
        [Fact]
        public void Day4a_SampleA1()
        {
            var result = GetBestGuardIdMinuteIndexByMostAsleep(ParseData(sampleA1));

            Assert.Equal(240, result);
        }
        
        private int GetBestGuardIdMinuteIndexByMostAsleep(IEnumerable<Guard> guards)
        {
            var sleepiestTotal = guards.Max(gu => gu.TotalSleep);
            var guard = guards.Single(g => g.TotalSleep == sleepiestTotal);
            return guard.GetSleepiestMinute().index * guard.Id;
        }

        private int GetBestGuardIdMinuteIndexByMostFrequentMinute(IEnumerable<Guard> guards)
        {
            var sleepiestMinute = guards.Max(g => g.GetSleepiestMinute().count);
            var guard = guards.Single(g => g.GetSleepiestMinute().count == sleepiestMinute);
            return guard.GetSleepiestMinute().index * guard.Id;
        }

        [Fact]
        public void Day4b_SampleA1()
        {
            var result = GetBestGuardIdMinuteIndexByMostFrequentMinute(ParseData(sampleA1));
            Assert.Equal(4455, result);
        }

        private readonly Regex _eventRegex = new Regex(@"\[(?<time>.*)\] (?<event>.*)", RegexOptions.Compiled);
        private readonly Regex _guardIdRegex = new Regex(@"(\d+)", RegexOptions.Compiled);
        ...
        [Fact]
        public void Day4a_Data()
        {
            var result = GetBestGuardIdMinuteIndexByMostAsleep(ParseData(dataString));

            Assert.True(false, $"Result is {result}");
        }

        [Fact]
        public void Day4b_Data()
        {
            var result = GetBestGuardIdMinuteIndexByMostFrequentMinute(ParseData(dataString));

            Assert.True(false, $"Result is {result}");
        }

        const string sampleA1 = @"[1518-11-01 00:00] Guard #10 begins shift
...
const string dataString = @"[1518-08-21 00:39] wakes up
